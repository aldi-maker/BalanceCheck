...
def run():
    st.title("Laporan RPL & ATM TAG")

    # --- Upload terpisah ---
    file_CIT_TAG = st.file_uploader("Upload laporan CIT (sheet CIT TAG)", type=["xlsx"])
    file_IC_TAG = st.file_uploader("Upload laporan IC (sheet IC TAG)", type=["xlsx"])
    file_wilayah_TAG = st.file_uploader(
        "Upload laporan Rencana & Wilayah(saldo Posisi Saldo TAG)", type=["xlsx"]
    )
    file_PosisiAwal_TAG = st.file_uploader(
        "Upload laporan Saldo awal TAG", type=["xlsx"]
    )

    if file_CIT_TAG and file_IC_TAG and file_wilayah_TAG and file_PosisiAwal_TAG:
        # --- 0. Detail CIT ---
        df_CIT = pd.read_excel(file_CIT_TAG, sheet_name="RECON", skiprows=11)
        ...
        # --- 1. Detail IC ---
        xls_IC = pd.ExcelFile(file_IC_TAG)
        ...
        df_IC = pd.read_excel(file_IC_TAG, sheet_name=selected_sheet, skiprows=1)
        df_IC.rename(
            columns={
                "Unnamed: 0": "NO",
                "Unnamed: 1": "WSID",
        ...
        df_cabang = pd.read_excel(file_wilayah_TAG, sheet_name="RENCANA", skiprows=1)
        ...
        # --- 3. Rekap per Wilayah ---
        wb = load_workbook(file_wilayah_TAG, data_only=True)
        ws = wb.active
        mapping = {
            "TAG TANGERANG": ["D4", "E4", "F4", "D5", "E5", "F5"],
            "TAG AMBON": ["G4", "H4", "I4", "G5", "H5", "I5"],
        }
        data = []
        ...
        wb = load_workbook(file_PosisiAwal_TAG, data_only=True)
        ws = wb.active
        mapping = {
            "TAG TANGERANG": ["D4", "E4", "F4"],
            "TAG AMBON": ["G4", "H4", "I4"],
        }
        data_realisasi = []
        for wilayah, cells in mapping.items():
        ...
        idatm_per_cabang = (
            df_selected_IC.groupby("Cabang")["WSID"]
            .apply(lambda x: set(x.dropna().astype(int).astype(str)))
            .to_dict()
        )
            ...
            if wsid_not_in_ic or idatm_not_in_rencana:
                ...
                if idatm_not_in_rencana:
                    ...
                    for atm_id in sorted(idatm_not_in_rencana):
                        row_ic = df_selected_IC.loc[df_selected_IC["WSID"] == atm_id]
                        if not row_ic.empty:
                            nominal_val = (
        ...
        def make_desc(row):
            if pd.notna(row["WSID"]) and row["Mismatch_Limit"] != 0:
                try:
                    atm_id = str(int(float(row["WSID"])))  # aman dari float NaN
                except Exception:
                    atm_id = str(row["WSID"])
